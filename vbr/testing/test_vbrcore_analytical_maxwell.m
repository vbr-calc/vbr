function TestResult = test_vbrcore_analytical_maxwell()
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TestResult = test_vbrcore_analytical_maxwell()
%
% test that maxwell_analytical runs with the different viscosity options
%
% Parameters
% ----------
% none
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    VBR.in.anelastic.maxwell_analytical.viscosity_method_mechanism = 'gbs';
    [VBR] = VBR_spine(VBR);
    TestResult = check_fdep_size(TestResult, VBR, fdep_size);
    ...
    VBR.in.anelastic.maxwell_analytical.eta_ss = 1e22;
    VBR.in.elastic.Gu_TP = full_nd(60*1e9, [fdep_size(1), fdep_size(2)]);
    VBR.in.elastic.quiet = 1;
    tau_M_expected = VBR.in.anelastic.maxwell_analytical.eta_ss ./ VBR.in.elastic.Gu_TP;

    [VBR] = VBR_spine(VBR);
    TestResult = check_fdep_size(TestResult, VBR, fdep_size);
    tau_M = VBR.out.anelastic.maxwell_analytical.tau_M;
    ...
    Qinv = VBR.out.anelastic.maxwell_analytical.Qinv;
...
function TestResult = check_fdep_size(TestResult, VBR, fdep_size)
    out_size = size(VBR.out.anelastic.maxwell_analytical.Qinv);
    if sum(out_size==fdep_size) ~= numel(fdep_size)
        TestResult.passed = false;
        msg = ['maxwell_analytical frequency dependent variables do not ', ...
