function sweep = generate_parameter_sweep_testing()
  % testing function for speeding up the VBR call
  ...
  sweep = generate_parameter_sweep_fast(sweep_params);
end

function sweep = generate_parameter_sweep_fast(sweep_params)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%               zmin,zmax,Nz    (optional) depth range min/max an number of z
%                               points between. Default to (0,300,150)
%
% Output:
% -------
...
% z range
if isfield(sweep_params,'zmin')
  zmin=sweep_params.zmin;
else
  zmin=0;
end
if isfield(sweep_params,'zmax')
  zmax=sweep_params.zmax;
else
  zmax=300;
end
if isfield(sweep_params,'Nz')
  Nz=sweep_params.Nz;
else
  Nz=150;
end
z = linspace(zmin,zmax,Nz)*1e3; z= z';


% set sweep parameter matrix
[T_K,phi,dg_um] = meshgrid(sweep_params.T,sweep_params.phi,sweep_params.gs);
VBR.in.SV.T_K=T_K;
VBR.in.SV.phi=phi;
VBR.in.SV.dg_um=dg_um;
disp('size of T,phi,dg matrix:')
disp(size(T_K))

% set constants
sz=size(T_K);
VBR.in.SV.sig_MPa = 0.1*ones(sz);
VBR.in.SV.Ch2o = zeros(sz); % in PPM!
VBR.in.SV.rho = 3300 * ones(sz); % [Pa]
VBR.in.SV.chi = ones(sz);
...
sweepBox = calculate_sweep(VBR, sweep_params, z);
...
function [sweepBox] = calculate_sweep(VBR, sweep_params, z)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% loop over depth, only change the pressure
% disp(fieldnames(VBR.in.SV))
n_z=numel(z);
i_z=n_z;
% for i_z=n_z:-1:1
  disp([num2str(i_z),' of ',num2str(n_z)])
  z_i=z(i_z);
  VBR.in.z = z_i;
  VBR.in.SV.P_GPa = z_i * 3300 * 9.8 /1e9*ones(sz);
  solidus_C = SoLiquidus(VBR.in.SV.P_GPa(1), 0,0,'hirschmann');
  VBR.in.SV.Tsolidus_K = (solidus_C.Tsol + 273)*ones(sz);

  % run VBR for current asthenospheric depths at all T,phi,dg
  VBR = VBR_spine(VBR);
...
function [sweepBox] = calculate_sweep_original(VBR_init, sweep_params)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% sweepBox = calculate_sweep(sweep_params);
%
